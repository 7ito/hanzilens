...
function logAiUsage(payload: Record<string, unknown>): void {
  ...
  captureAnalytics('provider_usage', {
    ...
    provider_route: enrichedPayload.route,
    feature: enrichedPayload.feature,
    provider_feature: enrichedPayload.providerFeature,
    usage_source: enrichedPayload.usageSource,
    ...
    max_tokens: enrichedPayload.maxTokens,
    estimated_cost_usd: enrichedPayload.estimatedCostUsd,
    ocr_units: enrichedPayload.units,
    input_bytes: enrichedPayload.inputBytes,
    image_width: enrichedPayload.imageWidth,
    image_height: enrichedPayload.imageHeight,
    extracted_chars: enrichedPayload.extractedChars,
...
async function extractLinesFromImage(imageDataUrl: string): Promise<OcrResult> {
  ...
  try {
    ...
    logAiUsage({
      ...
      providerFeature: 'TEXT_DETECTION',
      units: 1,
